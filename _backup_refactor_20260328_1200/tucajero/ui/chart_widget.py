...
class ChartWidget(QWidget):
    ...
    def plot_bar(self, labels, values, title="Ventas", color=None):
        """Gráfico de barras"""
        self.labels = labels
        self.values = values
        self.title = title
    ...
    def plot_pie(self, labels, values, title="Distribución", colors=None):
        """Gráfico de torta"""
        self.labels = labels
        self.values = values
        self.title = title
    ...
    def _draw_no_data(self):
        ...
        painter.drawText(
            self.rect(), Qt.AlignmentFlag.AlignCenter, "Sin datos disponibles"
        )
    ...
    def _draw_bar_chart(self):
        ...
        painter.drawText(
            0, 0, self.width(), 30, Qt.AlignmentFlag.AlignCenter, self.title
        )
        ...
        # Colores para barras
        bar_color = self.bar_color if self.bar_color else self._get_colors(1)[0]
        colors = [bar_color] * len(self.values)
        ...
        for i, (label, value) in enumerate(zip(self.labels, self.values)):
            ...
            painter.setPen(QPen(QColor(bar_color), 1))
            ...
            painter.drawText(
                int(x + bar_width / 2 - text_width / 2), int(y - 5), value_text
            )
            ...
            if len(label) > 8:
                painter.save()
                painter.translate(
                    int(x + bar_width / 2), int(margin_top + chart_height + 10)
                )
                painter.rotate(-45)
                ...
                painter.drawText(
                    int(x + bar_width / 2 - text_width / 2),
    ...
    def _draw_pie_chart(self):
        ...
        # Colores
        colors = (
            self.pie_colors if self.pie_colors else self._get_colors(len(self.values))
        )
        ...
        for i, (label, value) in enumerate(zip(self.labels, self.values)):
            ...
            text = (
                f"{label[:15]}... ({percentage:.1f}%)"
                if len(label) > 15
                else f"{label} ({percentage:.1f}%)"
            )
...
def get_ventas_por_periodo(session, periodo="mes"):
    ...
    if periodo == "dia":
        labels = [
            f"{int(r[0]):02d}:00" if r[0] is not None else "Sin hora"
            for r in resultados
        ]
    elif periodo == "mes":
...
def get_ventas_por_metodo(session, periodo="mes"):
    ...
    resultados = (
        ...
        .filter(
            Venta.metodo_pago.isnot(None), Venta.anulada == False, Venta.fecha >= inicio
        )
...
def get_ventas_por_cliente(session):
    ...
    anonimas = (
        ...
        .filter(Venta.cliente_id == None, Venta.anulada == False)
        .scalar()
    )
